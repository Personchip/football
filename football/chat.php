...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="msgsDialog" class="block">
            ...
            <label class="options first"><input id="autoScroll" type="checkbox" checked="checked" /> прокручивать
                вниз</label>
        ...
        <form action="" method="post" id="sendForm">
            <div id="sendDialog" class="block2">
                <input type="text" name="name" value="<?php echo ($name); ?>" maxlength="<?php echo (MAXUSERNAMELEN) ?>"
                    placeholder="Имя" />
                <textarea name="text" placeholder="Текст" style="margin-top: 0.5em;"
                    maxlength="<?php echo (MAXUSERTEXTLEN); ?>"></textarea>
            ...
            XMLo.onreadystatechange = function() {
                if (XMLo.readyState == 4) {
                    if (XMLo.status == 200 || XMLo.status == 0) {
                        handler(true, XMLo.status, XMLo.responseText, (XMLo.responseXML ? XMLo.responseXML
        ...
        function fade(o, opts, dontStartNow) {
            ...
            var fs = {
                ...
                stop: function(end, dontNotify) {
                    ...
                    if (dontNotify !== true && opts.hasOwnProperty("handler")) opts.handler(true, fs,
                        o);
                },
                ...
                start: function(restart, newOpts, dontNotify) {
                    ...
                    if (restart) {
                        init();
                        if (dontNotify !== true && opts.hasOwnProperty("handler")) opts.handler(false,
                            fs,
                            o);
        ...
        var tipUpper = (function() {
            ...
            return function(o, html, ax, ay) {
                ...
                function ae(obj, event, handler) {
                    if (typeof(obj.addEventListener) != 'undefined') obj.addEventListener(event,
                        handler, true);
                    else if (typeof(obj.attachEvent) != 'undefined') obj.attachEvent('on' + event,
                        handler, true);
                }

                function de(obj, event, handler) {
                    if (typeof(obj.removeEventListener) != 'undefined') obj.removeEventListener(
                        event,
                        handler, true);
                    else if (typeof(obj.detachEvent) != 'undefined') obj.detachEvent('on' + event,
                        handler);
                }
                ...
                t.innerHTML = '<div class="ugol"></div><div class="ugolI"></div><div class="msg">' +
                    html + '</div>';
                var c = getCords(o);
                ...
                post(
                    ...
                    function(state, status, txt) {
                        ...
                        if (txt !== undefined) {
                            var p = txt.indexOf("\n");
                            if (p > 0) {
                                var s = /^([a-z]+):(\d+)$/i.exec(txt.substring(0, p)),
                                    lm;
                                if (s) {
                                    lm = s[2];
                                    s = s[1];
